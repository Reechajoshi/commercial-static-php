...
CWin = window.prototype = {
	...
	_Z_INDEX_BASE: 940,  //THE ORDER RESOLVES FROM THIS UPWORDS
	_Z_INDEX_MODAL_INC: 10,
	_Z_INDEX_PIN: 1,
	...
	win: function(wid, parms)
	{
		...
		{		
			...
			var __zi = (CWin._Z_INDEX_BASE + (CWin._M_ALL_COUNT * CWin._Z_INDEX_MODAL_INC));
			//ZI = MAX (since it will be above everything except the win to be created below (which is MODAL)
			this._shtml += "<div name='" + this._nameModalID + "' class='vwin-max " + ((this._def.modalShade == CWin._M_SHADE_LINES)?("vwin-lines-shade"):((this._def.modalShade==CWin._M_SHADE_LIGHT)?("vwin-light-shade"):(''))) + "' style='left:0px;top:0px;width:" + CUi.get_view_cxy(true) + "px;height:" + (CUi.get_view_cxy(false) + 500) + "px;z-index:"+ __zi +"'>&#160;</div>"; 
		...
		var ___zi = (CWin._Z_INDEX_BASE + ((CWin._Z_INDEX_MODAL_INC * (1 + CWin._M_ALL_COUNT)) - 1) );
		this._shtml += "<div name='" + CWin._M_WIN_MAIN_NAME + "' class='vwin-full' style=\"background-color:" + this._def.winBGColor + ";position:fixed;z-index:" + ___zi + ";left:" + this._left + "px;top:" + this._top + "px;width:" + (this.totalWidth) + "px;height:" + this.totalHeight + "px;\">";
		this._ehtml += '</div>';
	...
	showBlock: function(blockid, wm_txt, wm_width, wm_height)
	{
		if(!CUtil.varok(wm_width))
			wm_width = 400;
			
		if(!CUtil.varok(wm_height))
			wm_height = 250;
			
		return(new CWin.win(CWin._M_BLOCK_ID + blockid, {
			withFrills: false,
			autoOpen: true,
			width: wm_width,
			height: ( (CUi._isIE) ? (wm_height+15) : (wm_height) ), //HACK: THIS COMES FROM IE'S BOX MODAL DUE TO CSS, OUCH
			modal: true,
			html: wm_txt,
			winCanResize: false,
			winCanMove: false,
...
CWin.prototype = CWin.win.prototype = {
	...
	initLeftTop: function()
	{
		if(this._def.positionMid)
		{
			...
			this._left = ( ( cw / 2 ) - (this.totalWidth / 2) ) + this._def.positionMidOffsetXY;
			this._top = ( ( ch / 2 ) - (this.totalHeight / 2) ) - this._def.positionMidOffsetXY;		
		}
	...
	hookModalRePos: function()
	{
		var me = this, cb = function() {
			if(me._elm.childNodes)
			{
				...
				me._elm.childNodes[0].style.width = CUi.get_view_cxy(true); //CUi.bod.clientWidth;
				me._elm.childNodes[0].style.height = CUi.get_view_cxy(false); //CUi.bod.clientHeight;	
				//me._elm.style.width = CUi.get_view_cxy(true); //CUi.bod.clientWidth;
				//me._elm.style.height = CUi.get_view_cxy(false); //CUi.bod.clientHeight;					
			}
			else
			{
	...
	setWinHeight: function(new_height)
	{
		wmain = this.getWinMain();
		wcon = this.getWinContent();
		
		this.applyParams( { height: new_height } );
	...
	hookMidPosCheck: function()
	{
		...
		CUi.hookResize(this._wid, function(e) {
			this.handleMidPosResize(me);
		} );
	},
	
	handleMidPosResize: function(me)
	{
		setTimeout( function() {
	...
	show: function()
	{
		if(this._def.modal)
		{
			this._def.fadeShade = true; // TODO: HACK: due to bug in obfuscator
	...
	destroy: function()
	{
		try
		{
			...
			if(doDestroy)
			{
				...
				if(CUtil.varok(CWin._M_ALL[this._wid]))
				{
