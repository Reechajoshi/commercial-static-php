CUi = {
	...
	_M_AGENT_IE: "MSIE",
	_M_AGENT_SF: "Safari",
	_M_AGENT_OP: "Opera", //only applicable for mobile blach hacks
	...
	browserDetect: function()
	{
		...
		CUi._isAndroid = false; CUi._isIOS = false; CUi._isAndroid_Opera = false;
		//CUi._isAndroid = true;
		//return ;
		
		if( ( chi = navigator.userAgent.indexOf(CUi._M_AGENT_IPAD) ) !=-1 )
		{
			CUi._isIOS = true; //SIMPLE
		}
		else if( ( chi = navigator.userAgent.indexOf(CUi._M_AGENT_ANDROID) ) !=-1 )
		{
			CUi._isAndroid = true;
			if(navigator.userAgent.indexOf(CUi._M_AGENT_OP) !=-1)
				CUi._isAndroid_Opera = true;
		...
		CUi._isTab = (CUi._isIOS || CUi._isAndroid);
	},
	...
	transformMe: function(ob, tf)
	{
		try {
			ob.style.Transform = tf;
			ob.style.MozTransform = tf;
			ob.style.webkitTransform  = tf;
			ob.style.msTransform = tf;
			ob.style.oTransform = tf;
		} catch(e) { }
	},
	
	applyStyles: function(sob, sstyle)
	{
		for( var pname in sstyle )
		{	
			if( pname.indexOf( 'transition' ) == 0 )
			{
				var pnbase = pname.substr(1);
				
				sob.style[ 'mozT' + pnbase ] = sstyle[ pname ];
				sob.style[ 'webkitT' + pnbase ] = sstyle[ pname ];
				sob.style[ 'oT' + pnbase ] = sstyle[ pname ];
				sob.style[ 'ieT' + pnbase ] = sstyle[ pname ];
			}
			
			sob.style[ pname ] = sstyle[ pname ];
		}
	},
	
	setTransitionOn: function(sob, sStartP, sEndP, sEndCB)
	{
		CUi.applyStyles( sob, sStartP );
		
		setTimeout( function() {
			CUi.applyStyles( sob, sEndP );
		}, 100 );
		
		if(sEndCB)
		{
			sEndCB = function() { alert('abc') };
			//alert(sob.webkitAnimationEnd);
			sob.webkitAnimationEnd = sEndCB;
			//sob.addEventListener("webkitAnimationEnd", sEndCB, false);
			//sob.addEventListener("animationend", sEndCB, false);
			//sob.addEventListener("oanimationend", sEndCB, false);
		}
	},
	...
	printArr: function(pv, x)
	{
		var loadedImgO = new Array();
		for(var xi in x)
		{
			CUi.log( xi + ': ' + JSON.stringify(x[xi]) );
		}
	},
	
	// NOTE: Will not work for images with the same file name (but under a different dir/ tree structure)
	loadImages: function(imgs, cb)
	{
		var loadedImgCount = 0, maxImgCount = -1;
		var functLoaded = function(o, bOK)
		{
			//if(bOK)
			//	storeO = { ok:true, img: o, width: o.width, height: o.height }
			//else
			//	storeO = { ok:false, img: o, width: o.width, height: o.height }
				
				//loadedImgO[ CUtil.basename(o.src) ] = ( storeO );
			
			if( CUtil.varok( o.getAttribute('name') ) )
			{
				imgs[ o.getAttribute('name') ].ok = true
				if((++loadedImgCount) >= maxImgCount)
					cb();
			}
		}
		
		//TODO: NEED to handle state etc..
		for(var ix in imgs)
		{
			imgs[ix].img.onload = function() { functLoaded(this,true); };
			imgs[ix].img.onerror = function() { functLoaded(this,false); };
			imgs[ix].img.setAttribute('name', ix);
			imgs[ix].img.src = imgs[ix].src;
		}
	},
	...
	showWorking: function()
	{
		...
		el.style.WebkitBorderRadius = '2px';
		el.style.MozBorderRadius = '2px';
